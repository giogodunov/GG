...
import { SectionCoverCustomizer, SectionCoverKey } from './SectionCoverCustomizer';

interface AdminPanelModalProps {
  isOpen: boolean;
  initialTab?: 'services' | 'tours' | 'guides' | 'inquiries' | 'settings';
  initialSectionCover?: SectionCoverKey;
  onClose: () => void;
  tours: Tour[];
  services: Service[];
  guides?: TravelGuide[];
...
export const AdminPanelModal: React.FC<AdminPanelModalProps> = ({
  ...
  const handleSaveSettings = (e: React.FormEvent) => {
    ...
    const updatedSettings: SiteSettings = {
      ...
      googleBusinessUrl: settingsForm.googleBusinessUrl?.trim() || 'https://share.google/ljHaCKoL7bDPdtIg3',
          ...
          {activeTab === 'settings' && (
            <div className="space-y-6 max-w-3xl">
              <form onSubmit={handleSaveSettings} className="bg-white p-6 rounded-2xl border border-stone-200 space-y-5">
                ...
                <div className="bg-gradient-to-br from-amber-50/70 via-white to-amber-50/40 border border-amber-200/90 rounded-2xl p-4 sm:p-5 space-y-3.5 shadow-xs">
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          <h4 className="text-xs font-bold text-amber-950">
                            Google Business & Reviews (ოფიციალური შეფასებების ბმული)
                          </h4>
                        ...
                        <p className="text-[11px] text-stone-600 mt-0.5">
                          გამოჩნდება საიტის მთავარ ბანერზე და Footer-ში, რაც უცხოელ ტურისტებში უდიდეს ნდობას ბადებს.
                        </p>
                      </div>
                    </div>

                    <a
                      href={settingsForm.googleBusinessUrl || 'https://share.google/ljHaCKoL7bDPdtIg3'}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="inline-flex items-center gap-1.5 text-xs font-semibold text-amber-800 hover:text-amber-950 bg-amber-100/80 hover:bg-amber-100 px-3 py-1.5 rounded-xl border border-amber-300/80 transition-all shrink-0 self-start sm:self-auto"
                    >
                      <ExternalLink className="w-3.5 h-3.5" />
                      <span>ბმულის შემოწმება</span>
                    </a>
                  </div>

                  <div>
                    <label className="block text-xs font-semibold text-stone-800 mb-1">
                      Google Business / Maps / Reviews გაზიარების ბმული (URL)
                    </label>
                    <div className="flex gap-2">
                      <input
                        ...
                        placeholder="https://share.google/ljHaCKoL7bDPdtIg3"
                        className="flex-1 px-3.5 py-2 text-xs bg-white border border-stone-200 rounded-xl focus:ring-1 focus:ring-amber-500 font-mono text-stone-800"
                      />
                      {settingsForm.googleBusinessUrl !== 'https://share.google/ljHaCKoL7bDPdtIg3' && (
                        <button
                          type="button"
                          onClick={() => setSettingsForm({ ...settingsForm, googleBusinessUrl: 'https://share.google/ljHaCKoL7bDPdtIg3' })}
                          className="px-3 py-2 text-xs font-medium text-stone-700 bg-stone-100 hover:bg-stone-200 rounded-xl transition-colors shrink-0"
                          title="აღადგინე In Georgia Tours-ის ბმული"
                        >
                          ნაგულისხმევი
                        </button>
                      )}
                    ...
                    <p className="text-[11px] text-stone-500 mt-1.5">
                      💡 შეგიძლიათ ჩასვათ Google Maps-იდან ან Google Search-იდან დაკოპირებული ნებისმიერი ბმული (მაგ: <code>https://share.google/ljHaCKoL7bDPdtIg3</code>).
                    </p>
                  </div>
                </div>
                ...
                {/* All Sections Background Cover Photos Customizer */}
                <div className="pt-4 border-t border-stone-100">
                  <SectionCoverCustomizer
                    settings={settingsForm}
                    onChangeSettings={(updated) => setSettingsForm(updated)}
                    initialSection={initialSectionCover || "hero"}
                  />
                </div>
