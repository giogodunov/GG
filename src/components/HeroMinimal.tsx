    ...
    >
      ...
      {hasCoverImage && (
        <div className="absolute inset-0 w-full h-full pointer-events-none overflow-hidden">
          ...
          <div className="hidden sm:block absolute inset-0 bg-gradient-to-l from-black/60 via-black/20 to-transparent" />
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex justify-end">
        <div className="max-w-2xl lg:max-w-3xl w-full flex flex-col items-start sm:items-end text-left sm:text-right">
          {/* Subtle label */}
          <div
            className={`inline-flex items-center gap-2 text-[10px] uppercase font-bold tracking-widest mb-3.5 px-3 py-1 rounded-full border transition-all ${
              isLightText
                ? 'text-white/90 bg-white/10 border-white/20 backdrop-blur-xs'
                : 'text-[#1A1A1A]/70 bg-black/5 border-black/10'
            }`}
          >
          ...
          <h1
            className={`text-4xl sm:text-6xl lg:text-7xl font-serif italic tracking-tight leading-[1.05] mb-4 sm:text-right ${
          ...
          {/* Subtext */}
          <p
            className={`text-base sm:text-lg max-w-xl font-normal leading-relaxed sm:text-right sm:ml-auto ${
              isLightText ? 'text-white/95 drop-shadow-xs' : 'text-[#1A1A1A]/70'
            }`}
          >
            {language === 'en'
          ...
          {/* CTAs */}
          <div className="mt-8 flex flex-wrap items-center gap-3 sm:justify-end w-full">
            <a
              href={directWhatsAppUrl}
              target="_blank"
              rel="noopener noreferrer"
              id="hero-whatsapp-btn"
              className="inline-flex items-center gap-2.5 bg-[#25D366] text-white px-6 py-3.5 rounded-xl font-bold text-xs tracking-wider uppercase hover:opacity-90 transition-all shadow-md hover:shadow-lg hover:-translate-y-0.5"
            >
            ...
            <button
              ...
              id="hero-form-btn"
              className={`inline-flex items-center gap-2 px-6 py-3.5 rounded-xl text-xs font-medium tracking-widest uppercase transition-all cursor-pointer shadow-md hover:shadow-lg hover:-translate-y-0.5 ${
                isLightText
